 ...
 * For each remaining vertex, it calculates the z position and weight of the tracks associated with it,
 * and determines whether they belong to the first or second new vertex.
 * The function then enters a loop where it iteratively updates the positions of the two new vertices
 * until convergence or a maximum number of iterations is reached.
 * After convergence, it checks if the new vertices are valid and if the chi-squared distance between them is significant enough.
 * If so, it assigns the tracks to the new vertex and updates the necessary data structures.
