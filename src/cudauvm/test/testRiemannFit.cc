 ...
 * performs fast fit, circle fit and line fit, and prints out 
 * the fitted parameters and chi squared values.
  
 * @brief Prepares broken line data from hits and initial fit results
 *
 * This function takes in hits, initial fit results, and curvature 
 * parameter B, and prepares the necessary data for 
 * subsequent fits.
 
 * @brief Performs fast fit on hits
 *
 * This function takes in hits and returns the fitted parameters 
 * as a vector of four doubles representing X0, Y0, R, and tan(theta).
 
 * @brief Performs line fit on hits and their covariance matrix
 *
 * This function takes in hits, their covariance matrix, 
 * initial fit results, and curvature parameter B, 
 * and returns the fitted parameters and chi squared value.
 
 * @brief Performs circle fit on hits and their covariance matrix
 *
 * This function takes in hits, their covariance matrix, 
 * initial fit results, and curvature parameter B, 
 * and returns the fitted parameters and chi squared value.
 
 * @brief Loads 2D covariance matrix from input matrix
 *
 * This function takes in an input matrix and loads its 
 * upper 2x2 submatrix into a 2D covariance matrix.
 
 * @brief Calculates radius from hit positions
 *
 * This function calculates the norm of the first two rows 
 * of the input matrix column-wise.
 
 * @brief Transforms circle fit parameters to par uv r tau kappa format
 *
 * This function transforms the input circle fit parameters 
 * to the par uv r tau kappa format.
 */void testFit() {
