...
/**
 * Enqueues a callback operation that waits for the specified queue to finish 
 * its tasks and then notifies the waiting task holder when done */    void ScopedContextHolderHelper::enqueueCallback(sycl::queue stream) {
      auto a = std::async([&]() {
        stream.wait();
