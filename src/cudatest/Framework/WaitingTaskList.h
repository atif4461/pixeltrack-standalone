 ...
 * signals when the resource is available for the waiting tasks to start.
  
 * Constructor initializing the WaitingTaskList object with an optional initial size parameter.
 
 * Member function to preset a task as failed before other tasks report failures.
 * @param iExcept exception pointer to be stored for later retrieval.
 
 * Adds a task to the waiting list, either holding it until the resource becomes available 
 * or spawning it immediately if the resource is already available.
 * @param iGroup task group associated with the task.
 * @param iTask task to be added to the waiting list.
 
 * Alternative overload to add a task to the waiting list.
 * @param iHolder WaitingTaskHolder object containing task information.
 
 * Signals that the resource is now available and waiting tasks should be spawned.
 * @param iPtr exception pointer to indicate success or failure.
 
 * Resets access to the resource, making newly added tasks wait.
 
 * Private helper function to handle task spawning, thread-safe for concurrent calls.
 
 * Nested struct representing a node in the waiting task list.
 * Contains pointers to the task, task group, and next node in the list, along with a flag indicating cache origin.
 
 * Creates a new WaitNode instance with the specified task group and task.
 * @param iGroup task group associated with the task.
 * @param iTask task to be encapsulated in the WaitNode.
 * @return Pointer to the newly created WaitNode.
 
 * Data members of the WaitingTaskList class, including head of the wait list, 
 * node cache, exception pointer, cache size, last assigned cache index, and waiting status flag.
