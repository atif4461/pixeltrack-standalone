...
/**
 * @brief Computes the scattering covariance line for a given set of parameters.
 *
 * @param cov_sz Pointer to an array of 2D matrices representing the covariance.
 * @param fast_fit Constant vector containing fit parameters.
 * @param s_arcs Constant vector of arc lengths.
 * @param z_values Constant vector of z values.
 * @param theta Constant angle parameter.
 * @param bField Constant magnetic field strength.
 * @param ret Reference to a matrix to store the result.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.2.
*/ 
...
/**
 * Calculates the covariance matrix in Cartesian coordinates from the covariance matrix in radial coordinates.
 *
 ...
 *
 * @param p2D input 2xN matrix
 * @param cov_cart input NxN cartesian covariance matrix
...
/**
 * Calculates the charge of a point with respect to a line segment defined by two points in 2D space.
 *
 * @param p2D 2xN matrix containing the coordinates of the line segment endpoints
 * @param par_uvr 3D vector representing the point to calculate the charge for
 ...
 * the best fit circle and line parameters. It uses various vector operations
 * and geometric calculations to determine the center and radius of the circle,
 * as well as the direction and pitch of the line.
 *
 * Parameters:
...
```cpp * @brief Brief description of the CircleFit function
 *
 * The CircleFit function calculates the parameters of a circle fit to a set of 2D hits
 * with their associated covariance matrices. It also computes the charge of the circle
 * and its chi-squared value. If the error flag is true, it propagates the errors of the
 * input hits to the circle parameters.
 *
 * @param hits2D Input 2D hits
 * @param hits_cov2D Covariance matrices of the input hits
 * @param fast_fit Initial guess for the circle fit
 * @param rad Radii of the hits
 * @param bField Magnetic field strength
 * @param error Flag to propagate errors
 * @return CircleFit object containing the fitted circle parameters and their covariance matrix
...
/**
 * @brief Structure to hold the result of a line fit.
 
struct LineFit {
     * @brief Parameters of the line fit.
   
  Eigen::Matrix<double, 2, 1> par;

     * @brief Covariance matrix of the line fit parameters.
   
  Eigen::Matrix<double, 2, 2> cov;

     * @brief Chi-squared value of the line fit.
   
  double chi2;
}; 
 * @brief Function to perform a line fit on a set of points.
 *
 * @param hits Set of points to perform the line fit on.
 * @param hits_ge Errors on the points.
 * @param circle Circle fit object.
 * @param fast_fit Fast fit object.
 * @param bField Magnetic field strength.
 * @param error Flag to indicate whether to compute errors.
 *
 * @return Line fit result.
 
__host__ __device__ inline LineFit lineFit(const M3xN& hits, const M6xN& hits_ge, const CircleFit& circle, const V4& fast_fit, const double bField, const bool error);* This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.2.
*/ 
...
*/
  ...
  template <int N>
/**
 * Class representing a helical fit result.
  
class HelixFit {
     * Constructor for HelixFit class.
   *
   * @param hits input data for the fit
   * @param hits_ge additional geometric data for the fit
   * @param bField magnetic field strength
   * @param error flag indicating whether to include error estimates
   
}; 
 * Class representing a circle fit result.
  
class CircleFit {
     * Constructor for CircleFit class.
   *
   * @param hits input data for the fit
   * @param hits_cov covariance matrix for the input data
   * @param fast_fit initial estimate for the fit parameters
   * @param rad radii of the input data points
   * @param bField magnetic field strength
   * @param error flag indicating whether to include error estimates
   
}; 
 * Class representing a line fit result.
  
class LineFit {
     * Constructor for LineFit class.
   *
   * @param hits input data for the fit
   * @param hits_ge additional geometric data for the fit
   * @param circle circle fit result to refine
   * @param fast_fit initial estimate for the fit parameters
   * @param bField magnetic field strength
   * @param error flag indicating whether to include error estimates
