...
/**
 * Brief description: This function splits vertices based on certain conditions.
 *
 * Detailed description:
 * The function takes three parameters: ZVertices pointer pdata, WorkSpace pointer pws, and float maxChi2.
 * It iterates over each vertex and checks if the number of tracks associated with it is less than 4 or 
 * if the chi-squared value is less than the product of maxChi2 and the number of degrees of freedom.
 * If either condition is met, it skips to the next vertex.
 * For each remaining vertex, it allocates shared memory to store track indices, z positions, 
 * new vertex flags, and weights.
 * It then copies data from global memory to shared memory and synchronizes threads.
 * The function enters a loop where it calculates new vertex positions and updates the new vertex flags.
 * Once the loop ends, it checks if the new vertices meet certain quality criteria and 
 * updates the global vertex index if necessary.
 *
 * Parameters:
 * @param pdata Pointer to ZVertices struct containing vertex data
 * @param pws Pointer to WorkSpace struct containing workspace data
 * @param maxChi2 Maximum allowed chi-squared value
