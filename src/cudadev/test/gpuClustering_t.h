...
```cpp * @brief This is the main function where the program starts execution.
 *
 * @return An integer indicating the program's exit status.
 
int main(void)
 * @class CablingWrapper
 * @brief A wrapper class for managing cabling maps on the GPU.
 *
 * This class provides methods for allocating and deallocating memory on the GPU,
 * as well as advising the GPU about memory usage patterns.
 
class CablingWrapper
 * @class gpuClustering
 * @brief A namespace for GPU-based clustering algorithms.
 *
 * This namespace contains functions and classes related to clustering pixels on the GPU.
 
 * @fn generateClusters
 * @brief A lambda function that generates pixel clusters.
 *
 * This function populates arrays with pixel data, including IDs, positions, and ADC values.
 
auto generateClusters = [&]()
 * @fn countModules
 * @brief A function that counts the number of modules.
 *
 * This function takes arrays of pixel IDs, module starts, and cluster assignments as input.
 
void countModules(h_id.get(), h_moduleStart.get(), h_clus.get(), n);
 * @fn findClus
 * @brief A function that finds clusters.
 *
 * This function takes arrays of pixel IDs, positions, module starts, and cluster assignments as input.
 
void findClus(h_id.get(), h_x.get(), h_y.get(), h_moduleStart.get(), h_clusInModule.get(), h_moduleId.get(), h_clus.get(), n);
 * @fn clusterChargeCut
 * @brief A function that applies a charge cut to clusters.
 *
 * This function takes threshold values, arrays of pixel IDs, ADC values, module starts, and cluster assignments as input.
 
void clusterChargeCut(clusterThresholds, h_id.get(), h_adc.get(), h_moduleStart.get(), h_clusInModule.get(), h_moduleId.get(), h_clus.get(), n);
