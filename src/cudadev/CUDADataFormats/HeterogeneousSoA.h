...
template <typename T, typename Traits>
/**
 * Returns a host unique pointer to the heterogeneous SoA implementation
 * asynchronously copies data from device to host using the specified stream
 *
 * @param stream The CUDA stream used for asynchronous copy operation
 * @return A host unique pointer to the copied data
