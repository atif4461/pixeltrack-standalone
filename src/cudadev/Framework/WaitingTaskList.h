 ...
 * Constructor for WaitingTaskList setting its initial size.
 * @param iInitialSize Initial size of the cache for holding waiting tasks.
 
 * Adds a task to the waiting list, either spawning it if the resource is available or holding it until doneWaiting is called.
 * @param iGroup Task group associated with the task.
 * @param iTask Task to be added to the waiting list.
 
 * Alternative method to add a task to the waiting list.
 * @param iHolder Holder containing the task to be added.
 
 * Signals that the resource is now available, allowing waiting tasks to spawn.
 * @param iPtr Exception pointer in case a task failed, defaults to nullptr.
 
 * Resets access to the resource, making newly added tasks wait.
 
 * Private method handling the spawning of tasks, safe for concurrent access.
 
 * Struct representing a node in the linked list of waiting tasks.
 * Contains pointers to the task and its associated task group, along with atomic next node pointer and flag indicating origin from cache.
 
 * Creates a new node for the waiting task list.
 * @param iGroup Task group associated with the task.
 * @param iTask Task being added to the waiting list.
 * @return Pointer to the newly created node.
 
 * Data members of the WaitingTaskList class, including head of the linked list, node cache, exception pointer, cache size, last assigned cache index, and waiting status flag.
