 ...
 * Adds a task to the waiting list, either immediately spawning it if the resource is available or holding it until doneWaiting is called.
 * Concurrent calls to add and doneWaiting are allowed.
 
 * Alternative method to add a task to the waiting list, taking a WaitingTaskHolder instead of separate task group and task pointers.
 * Concurrent calls to add and doneWaiting are allowed.
 ...
 * Resets access to the resource, making added tasks wait again.
 ...
 * Struct representing a node in the waiting list, containing a task, its associated task group, and a pointer to the next node in the list.
  
 * Method creating a new node in the waiting list for a given task group and task.
