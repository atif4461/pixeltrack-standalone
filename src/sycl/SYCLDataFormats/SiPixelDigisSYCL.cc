...
/**
 * Constructor initializing member variables with unique pointers 
 * to arrays on the device for storing pixel digi data.
 *
 * @param maxFedWords Maximum number of words per federated event
 * @param stream SYCL queue object representing the command queue
 */ 
...
// The following comment is generated by an LLM. 
/**
 * @brief Asynchronously copies ADC values from device to host memory
 * @param stream SYCL queue object
 * @return Unique pointer to array of uint16_t values on host
 */ 
...
// The following comment is generated by an LLM. 
/**
 * Retrieves the raw ID array from the device and copies it to the host asynchronously.
 *
 * @param stream The SYCL queue used for the operation.
 * @return A unique pointer to the raw ID array on the host.
 */ 
