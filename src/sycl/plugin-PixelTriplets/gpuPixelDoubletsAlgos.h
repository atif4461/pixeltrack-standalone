 ...
 *
 * This function takes in various inputs such as layer pairs, number of pairs, 
 * cell neighbors, cell tracks, tracking rec hit 2D SOA view, and more. It 
 * generates doublets based on certain conditions and cuts.
 *
 * @param layerPairs Array of layer pairs.
 * @param nPairs Number of pairs.
 * @param cells Pointer to array of GPUCACell objects.
 * @param nCells Pointer to integer storing number of cells.
 * @param cellNeighbors Pointer to CellNeighborsVector object.
 * @param cellTracks Pointer to CellTracksVector object.
 * @param hh Const reference to TrackingRecHit2DSOAView object.
 * @param isOuterHitOfCell Pointer to OuterHitOfCell array.
 * @param phicuts Pointer to array of phi cuts.
 * @param minz Pointer to array of minimum z values.
 * @param maxz Pointer to array of maximum z values.
 * @param maxr Pointer to array of maximum r values.
 * @param idealCond Boolean indicating ideal condition.
 * @param doClusterCut Boolean indicating whether to apply cluster cut.
 * @param doZ0Cut Boolean indicating whether to apply z0 cut.
 * @param doPtCut Boolean indicating whether to apply pt cut.
  ...
  __attribute__((always_inline)) void doubletsFromHisto(uint8_t const* __restrict__ layerPairs,
    ...
    for (auto j = idy; j < ntot; j += item.get_local_range().get(1) * item.get_group_range(1)) {
      ...
      */
      ...
      // bool piWrap = sycl::abs(kh-kl) > Hist::nbins()/2;
