...
/**
 * Constructor initializing SiPixelDigisCUDA object with maximum number of fed words and CUDA stream
 */
 ...
 * representing pixel digis transferred from device memory to host memory asynchronously
 */
// The above comment was written by an LLM. 
...
/**
 * @brief Returns a host unique pointer to an array of integers containing cluster data transferred from device memory asynchronously
 * @param stream CUDA stream handle
 * @return Unique host pointer to integer array
 */
// The above comment was written by an LLM. 
 ...
 * @return unique pointer to the copied raw ID array on the host
 */
// The above comment was written by an LLM. 
