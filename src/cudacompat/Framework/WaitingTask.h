...
class WaitingTask  * Constructor for WaitingTask.
 
WaitingTask * Destructor for WaitingTask.
 
~WaitingTask * Returns exception thrown by dependent task.
 * @return pointer to exception if dependent task failed, null otherwise.
 
std::exception_ptr const* exceptionPtr * Called when waited for task fails, allowing exception transfer.
 * @param iPtr exception pointer from dependent task.
 
void dependentTaskFailed * Atomic pointer to exception data.
 
std::atomic<std::exception_ptr*> m_ptr* This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
 ...
 * @brief A waiting task that executes a functor when run
 * @details This class represents a task that waits until it is executed 
 *          and then runs a stored functor
