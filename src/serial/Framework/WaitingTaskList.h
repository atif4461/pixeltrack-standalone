...
namespace edm {
/**
 * Class representing a list of tasks that are waiting to be executed.
 * It allows tasks to be added and removed from the list, and signals when the tasks can start executing.
  
 * Class containing functionality for managing a list of waiting tasks.
 * Provides methods for adding tasks, signaling when tasks can start, and resetting the list.
  
 * Structure representing a node in the waiting task list.
 * Contains pointers to the task and its associated task group, as well as a pointer to the next node in the list.
  
 * Class responsible for handling the execution of waiting tasks.
 * Allows tasks to be added and executed in a thread-safe manner.
