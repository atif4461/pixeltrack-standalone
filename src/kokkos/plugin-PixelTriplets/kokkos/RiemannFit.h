...
/**
 * Computes radiation lengths of a pixel detector under uniform material assumption.
 *
 * @param[in] length_values Input values representing lengths.
 * @param[out] rad_lengths Output radiation lengths computed from input values.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION void computeRadLenUniformMaterial(const VNd1& length_values, VNd2& rad_lengths) {
...
/**
 * @brief Computes the scatter covariance line for a given set of parameters.
 *
 * This function calculates the scatter covariance line based on the input parameters,
 * including the covariance matrices, fit values, arc lengths, z-values, theta, and B.
 * It returns the computed covariance matrix.
 *
 * @param cov_sz   Array of 2x2 covariance matrices
 * @param fast_fit  Fit values
 * @param s_arcs   Arc lengths
 * @param z_values Z-values
 * @param theta   Angle theta
 * @param B       Value B
 * @param ret     Output covariance matrix
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION auto Scatter_cov_line(Matrix2d const* cov_sz,
                                                 const V4& fast_fit,
                                                 VNd1 const& s_arcs,
                                                 VNd2 const& z_values,
                                                 const double theta,
...
/**
 * Computes the scattering covariance radial matrix.
 *
 * @param p2D input 2D points
 * @param fast_fit fit parameters
 * @param rad radii values
 * @param B scaling factor
 * @return scattering covariance radial matrix
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION MatrixNd<N> Scatter_cov_rad(const M2xN& p2D,
                                                       const V4& fast_fit,
...
/**
 * Calculates the covariance matrix in cartesian coordinates from radial coordinates.
 *
 * @param p2D input 2xN matrix
 * @param cov_rad input NxN covariance matrix in radial coordinates
 * @param rad input Nx1 vector of radii
 * @return 2Nx2N covariance matrix in cartesian coordinates
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION Matrix2Nd<N> cov_radtocart(const M2xN& p2D,
...
*/
    template <typename M2xN, int N>
/**
 * Calculates the radial covariance from cartesian coordinates.
 *
 * @param p2D input 2D vector
 * @param cov_cart input cartesian covariance matrix
 * @param rad input radial vector
 * @return calculated radial covariance vector
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION VectorNd<N> cov_carttorad(const M2xN& p2D,
...
*/
    template <typename M2xN, typename V4, int N>
/**
 * Calculates the radial component of the covariance matrix 
 * from cartesian coordinates.
 *
 * @param p2D input 2D points
 * @param cov_cart input cartesian covariance matrix
 * @param fast_fit intermediate fit results
 * @param rad input radii
 * @return radial component of the covariance matrix
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION VectorNd<N> cov_carttorad_prefit(const M2xN& p2D,
                                                            const Matrix2Nd<N>& cov_cart,
                                                            V4& fast_fit,
                                                            const VectorNd<N>& rad) {
      constexpr u_int n = N;
...
*/
    ...
    template <int N>
    KOKKOS_INLINE_FUNCTION VectorNd<N> Weight_circle(const MatrixNd<N>& cov_rad_inv) {
...
/**
 * Calculates the charge of a point with respect to a line segment defined by two points.
 *
 * @param p2D a 2xN matrix containing the coordinates of the line segment
 * @param par_uvr a 3D vector representing the point to calculate the charge for
 * @return the charge of the point, either -1 or 1
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION int32_t Charge(const M2xN& p2D, const Vector3d& par_uvr) {
...
*/
    ...
    KOKKOS_INLINE_FUNCTION Vector3d min_eigen3D(const Matrix3d& A, double& chi2) {
#ifdef RFIT_DEBUG
...
*/
    ...
    KOKKOS_INLINE_FUNCTION Vector3d min_eigen3D_fast(const Matrix3d& A) {
...
*/
    ...
    KOKKOS_INLINE_FUNCTION Vector2d min_eigen2D(const Matrix2d& A, double& chi2) {
...
/**
 * Brief description: Fast fit function to calculate circle and line fits.
 *
 * Detailed description: This function takes in a set of 3D hits and calculates 
 * the best fit circle and line parameters. It uses various mathematical 
 * operations such as vector calculations and trigonometric functions to 
 * determine the center and radius of the circle, as well as the direction 
 * and pitch of the line.
 *
 * Parameters:
 *   hits (M3xN): A matrix containing the 3D coordinates of the hits.
 *   result (V4): A vector to store the calculated fit parameters.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION void Fast_fit(const M3xN& hits, V4& result) {
...
*/
    template <typename M2xN, typename V4, int N>
/**
 * @brief Class representing a circle fit result.
...
class CircleFitResult {
  ...
  ~CircleFitResult();
     ...
     * @brief Get the parameters of the fitted circle.
   * @return The parameters of the fitted circle.
   
  Vector3d getParameters() const;

     * @brief Get the charge of the fitted circle.
   * @return The charge of the fitted circle.
   
  double getCharge() const;

     * @brief Get the chi-squared value of the fitted circle.
   * @return The chi-squared value of the fitted circle.
   
  double getChiSquared() const;

     * @brief Get the covariance matrix of the fitted circle.
   * @return The covariance matrix of the fitted circle.
   
  Matrix3d getCovarianceMatrix() const;

private:
  Vector3d par_; ///< Parameters of the fitted circle.
  double q_; ///< Charge of the fitted circle.
  double chi2_; ///< Chi-squared value of the fitted circle.
  Matrix3d cov_; ///< Covariance matrix of the fitted circle.
}; 
 * @brief Function to perform a circle fit.
 * @param hits2D The 2D hit coordinates.
 * @param hits_cov2D The 2D hit covariance matrices.
 * @param fast_fit The initial estimate of the circle parameters.
 * @param rad The radii of the hits.
 * @param B The magnetic field strength.
 * @param error Whether to propagate errors.
 * @return The result of the circle fit.
 
CircleFitResult circle_fit(const M2xN& hits2D, const Matrix2Nd<N>& hits_cov2D, const V4& fast_fit, const VectorNd<N>& rad, const double B, const bool error);* This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION circle_fit Circle_fit(const M2xN& hits2D,
...
/**
 * @brief Structure representing a line fit result.
 *
 * This struct contains the fitted parameters, their covariance matrix, and the Chi-squared value of the fit.
 
struct line_fit {
     * @brief Fitted parameters of the line (m, q).
   
  Vector2d par;

     * @brief Covariance matrix of the fitted parameters.
   
  Matrix2d cov;

     * @brief Chi-squared value of the fit.
   
  double chi2;
}; 
 * @brief Function performing a line fit on a set of points.
 *
 * This function takes in a set of points, their geometric uncertainties, a circle fit object, 
 * a fast fit vector, a magnetic field strength, and an error flag. It returns a line_fit object containing 
 * the fitted parameters, their covariance matrix, and the Chi-squared value of the fit.
 *
 * @param hits Set of points to perform the fit on.
 * @param hits_ge Geometric uncertainties of the points.
 * @param circle Circle fit object.
 * @param fast_fit Fast fit vector.
 * @param B Magnetic field strength.
 * @param error Flag indicating whether to compute errors.
 * @return line_fit object containing the fit results.
 
line_fit Line_fit(const M3xN& hits, const M6xN& hits_ge, const circle_fit& circle, const V4& fast_fit, const double B, const bool error);* This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
    KOKKOS_INLINE_FUNCTION line_fit Line_fit(const M3xN& hits,
                                             const M6xN& hits_ge,
...
/**
 * @class HelixFit 
 * @brief Performs a helix fit on a set of input data points
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
