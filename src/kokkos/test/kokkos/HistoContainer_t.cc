 ...
 *
 * This function initializes random number generators, creates views for device memory,
 * and performs multiple iterations of histogram filling and verification.
 
void go()
 * @brief Verifies the ordering of two elements in the histogram.
 *
 * This lambda function checks if the difference between two elements is within a certain range.
 *
 ...
 * @brief Increments the index modulo the number of bins.
 *
 * This lambda function increments the input index and returns the result modulo the number of bins.
 *
 * @param k Index to increment.
 * @return Incremented index.
 
auto incr = [](auto& k) 
 * @brief Fills the histogram with data from a vector.
 *
 * This function fills the histogram with data from a vector, using the provided offset and size.
 *
 * @param hist Histogram to fill.
 * @param nParts Number of parts in the histogram.
 * @param vec Vector containing the data.
 * @param offs Offset array.
