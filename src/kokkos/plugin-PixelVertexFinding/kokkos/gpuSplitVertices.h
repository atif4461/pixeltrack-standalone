...
/**
 * Brief description: Splits vertices based on certain conditions.
 *
 * Detailed description: This function takes in several parameters including 
 * vertex data, workspace, maximum Chi2 value, and team member information. 
 * It iterates over each vertex, checks certain conditions such as the number 
 * of tracks associated with the vertex and the Chi2 value, and splits the 
 * vertex into two new ones if necessary. The splitting process involves 
 * calculating new vertex positions and weights, and updating the 
 * corresponding track information.
 *
 * Parameters:
 *   vdata - Vertex data
 *   vws - Workspace
 *   maxChi2 - Maximum Chi2 value
...
/**
 * Brief function to invoke vertex splitting operation on device memory
 * 
 * @param[in] vdata input vertices data
 * @param[in,out] vws workspace for vertex splitting
 * @param[in] maxChi2 maximum chi squared value
