...
/**
 * @brief Clusters tracks by density using the DBSCAN algorithm.
 *
 * This function takes in a set of tracks and their corresponding workspace data,
 * and groups them into clusters based on their density and proximity.
 *
 * The clustering process involves several steps:
 * 1. Counting the number of neighbors for each track within a certain radius.
 * 2. Finding the closest track with higher density for each track.
 * 3. Consolidating the graph by percolating the index of the seed track.
 * 4. Marking tracks as noise or assigning them to a cluster.
 *
 * The function returns the number of clusters found and assigns a unique ID to each cluster.
 *
 * @param vdata View of track data.
 * @param vws View of workspace data.
 * @param minT Minimum number of neighbors required to form a dense region.
 * @param eps Maximum distance between two tracks in a cluster.
 * @param errmax Maximum error allowed for a track to be considered part of a cluster.
 * @param chi2max Maximum normalized distance between two tracks in a cluster.
 * @param team_member Team member object for parallel execution.
...
/**
 * @brief Clusters tracks by density on the host
 * 
 * @param vdata input vertices data
 * @param vws input workspace data
 * @param minT minimum number of neighbors required to be a seed
 * @param eps maximum absolute distance to cluster
 * @param errmax maximum error allowed to be a seed
