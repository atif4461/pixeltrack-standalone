 ...
 * This function puts a product into the EventSetup
 * @param prod a unique pointer to the product to be stored
...
*/ 
    void put(std::unique_ptr<T> prod) {
      ...
      if (not succeeded.second) {
        throw std::runtime_error(std::string("EventSetup::put(): Product of type ") + typeid(T).name() +
                                 " already exists");
      }
    }
...
*/ 
    T const& get() const {
      ...
      if (found == typeToProduct_.end()) {
        throw std::runtime_error(std::string("EventSetup::get(): Product of type ") + typeid(T).name() +
                                 " is not produced");
      }
