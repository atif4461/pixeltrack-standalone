...
namespace allocator {
/**
 * Calculates the power of a number using exponentiation by squaring method
...
/**
 * Brief description of GenericCachingAllocator class
 *
 * Detailed description of GenericCachingAllocator class
 * This class implements a caching allocator that manages memory blocks
 * of varying sizes and provides features such as block growth and caching
...
/**
 * Comparison function for BlockDescriptor objects based on pointer values.
 * 
 * @param a First BlockDescriptor object to compare.
 * @param b Second BlockDescriptor object to compare.
 * @return True if the pointer value of a is less than the pointer value of b, false otherwise.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
...
/**
 * @brief Attempts to reuse a cached block based on the provided search key.
 *
 * This function iterates over the cached blocks on the same device and in the same bin,
 * checking if the block can be reused in the specified queue or if its event has completed.
 * If a reusable block is found, it is inserted into the live blocks, updated accordingly,
 * and its pointer is returned. Otherwise, a null pointer is returned.
 *
 * @param searchKey The descriptor of the block to search for.
 * @return A pointer to the reused block, or nullptr if none was found.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
...
/**
 * @brief Releases cached memory blocks associated with a specific device.
 *
 * This function iterates over the cached blocks, identifies those belonging to the specified device,
 * and frees their memory. It also updates internal tracking metrics and provides optional debug output.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
 ...
 *
 * This function releases all cached memory blocks, freeing their associated memory and destroying any related events.
 *
 * @note This operation is thread-safe due to the use of a scoped lock.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
  void freeAllCached() {
    std::scoped_lock lock(mutex_);
