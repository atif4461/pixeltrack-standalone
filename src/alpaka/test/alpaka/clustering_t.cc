...
1. `int main(void)` * Main program entry point.
 *
...
2. `auto generateClusters = [&](int kn)` * Generates test clusters based on input parameter kn.
 *
 * Creates various types of clusters, including big noise, isolated pixels, and more complex patterns.
 *
 * @param kn Input parameter controlling the type of clusters generated.
 

3. `alpaka::enqueue(queue, alpaka::createTaskKernel<Acc1D>(...))`  * Launches a kernel for counting modules.
 *
 * Counts the number of modules in the input data.
 *
 * @param queue The command queue for the operation.
 * @param workDivCountModules Work division for the kernel launch.
 * @param d_id Input data buffer containing pixel IDs.
 * @param d_moduleStart Output buffer for module start indices.
 * @param d_clus Output buffer for cluster assignments.
 * @param n Number of input pixels.
 

4. `alpaka::enqueue(queue, alpaka::createTaskKernel<Acc1D>(...))`  * Launches a kernel for finding clusters.
 *
 * Assigns pixels to clusters based on their positions and IDs.
 *
 * @param queue The command queue for the operation.
 * @param workDivMaxNumModules Work division for the kernel launch.
 * @param d_id Input data buffer containing pixel IDs.
 * @param d_x Input data buffer containing pixel X coordinates.
 * @param d_y Input data buffer containing pixel Y coordinates.
 * @param d_moduleStart Input buffer containing module start indices.
 * @param d_clusInModule Output buffer for cluster counts per module.
 * @param d_moduleId Output buffer for module IDs.
 * @param d_clus Output buffer for cluster assignments.
 * @param n Number of input pixels.
 

5. `alpaka::enqueue(queue, alpaka::createTaskKernel<Acc1D>(...))`  * Launches a kernel for applying a charge cut to clusters.
 *
 * Filters out clusters based on their total charge.
 *
 * @param queue The command queue for the operation.
 * @param workDivMaxNumModules Work division for the kernel launch.
 * @param d_id Input data buffer containing pixel IDs.
 * @param d_adc Input data buffer containing pixel ADC values.
 * @param d_moduleStart Input buffer containing module start indices.
 * @param d_clusInModule Input buffer containing cluster counts per module.
 * @param d_moduleId Input buffer containing module IDs.
 * @param d_clus Output buffer for updated cluster assignments.
 * @param n Number of input pixels.
 */int main(void) {
  initialise();
