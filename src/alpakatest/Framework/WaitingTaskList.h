 ...
 * or holding it until the resource becomes available.
 ...
 * If an exception occurred, it should be passed as a non-null stdexceptionptr.
 ...
 * This method is not thread-safe and should only be called when no tasks are using the resource and no concurrent calls to add or doneWaiting are being made.
 
 * Private method handling the spawning of tasks, safe to call from multiple threads.
 
 * Struct representing a node in the waiting list, containing a task, its associated task group, and a pointer to the next node.
 
 * Creates a new node in the waiting list, associating it with a task group and a task.
 
 * Member variable representing the head of the waiting list.
