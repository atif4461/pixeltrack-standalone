...
/**
 * Calculates the power of a number using exponentiation by squaring algorithm
 ...
 *
 * This class provides a way to manage memory allocation on a device, allowing for efficient reuse of previously allocated memory blocks.
 * It takes into account various parameters such as bin growth factor, minimum and maximum bin sizes, and maximum cached bytes.
 *
 * @param device The device on which the allocator operates.
 * @param binGrowth The factor by which the size of each bin grows.
 ...
 * @param maxCachedBytes The total storage available for the allocator, or zero for no limit.
 * @param maxCachedFraction The fraction of total device memory available for the allocator, or zero for no limit.
...
/**
 * Class representing a block reuser for caching purposes.
 
class BlockReuser {
     * Tries to reuse a cached block for a given block descriptor.
   *
 ...
 * Class representing a cached block manager.
 
class CachedBlockManager {
     * Manages a collection of cached blocks.
   
};
 * Class representing an event handler.
 
class EventHandler {
     * Handles events related to block caching.
   
};
 * Class representing a queue manager.
 
class QueueManager {
     * Manages queues for block allocation and deallocation.
   
};
 * Class representing a device manager.
 
class DeviceManager {
     * Manages devices for block allocation and caching.
   
};
 * Class representing a memory accountant.
 
class MemoryAccountant {
     * Keeps track of memory usage for cached blocks.
   
};
 * Class representing a debugger.
 
class Debugger {
     * Provides debugging functionality for block caching.
 ...
 * @param queue The queue associated with the device where the buffer will be allocated.
 ...
 *
 * This function releases all cached memory blocks, updating the total amount of free and live bytes accordingly.
 * It also provides debug output with detailed information about the number of available and live blocks, as well as the total bytes allocated.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
    void freeAllCached() {
      std::scoped_lock lock(mutex_);
