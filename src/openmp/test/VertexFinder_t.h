 ...
 * This class represents an event generator that populates an Event object with simulated data.
 * It generates clusters, tracks, and vertices, and adds noise to the generated data.
...
/**
 * Brief description of function to print vertex finder data
 *
 * Detailed description of function to print vertex finder data
 *
...
### Class gpuVertexFinder * @brief This class represents a GPU-based vertex finder.
 * It provides methods for initializing and fitting vertices on the GPU.
 
class gpuVertexFinder {
     * @brief Initializes the vertex finder.
   
  void init();

     * @brief Fits vertices using the provided work space.
   * @param ws Work space object.
   
  void fitVertices(gpuVertexFinder* onGPU_d, WorkSpace* ws_d, float parameter);

     * @brief Splits vertices using the provided work space.
   * @param ws Work space object.
   
  void splitVertices(gpuVertexFinder* onGPU_d, WorkSpace* ws_d, float parameter);

     * @brief Sorts vertices by Pt2.
   * @param ws Work space object.
   
  void sortByPt2(gpuVertexFinder* onGPU_d, WorkSpace* ws_d);
};

### Class WorkSpace * @brief This class represents a work space for vertex finding.
 * It provides methods for initializing and accessing work space data.
...
class WorkSpace {
  ...
  void init();
     ...
     * @brief Gets the number of intermediate vertices.
   * @return Number of intermediate vertices.
   
  uint32_t getNvIntermediate();
};

### Class ClusterGenerator * @brief This class generates clusters for testing vertex finding.
 * It provides a method for generating an event.
 
class ClusterGenerator {
     * @brief Generates an event.
   * @param nav Average number of vertices.
   * @param parameter Additional parameter.
   
  ClusterGenerator(int nav, int parameter);

     * @brief Generates an event.
   * @param ev Event object to generate.
   
  void generate(Event& ev);
};

### Class Event * @brief This class represents an event in the vertex finding process.
 * It contains data such as tracks and vertices.
 
class Event {
     * @brief Gets the track data.
   * @return Track data.
   
  std::vector<float> getTrackData();
