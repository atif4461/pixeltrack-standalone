 ...
 *
 * This function generates hits and their covariance matrices,
 * performs fast fit, circle fit and line fit, and prints out the results
 
void testFit();
 * @brief Fills hits and hits covariance matrices
 *
 * @param[out] hits matrix of hits
 * @param[out] hits_ge matrix of hits covariance
 
void fillHitsAndHitsCov(Rfit::Matrix3xNd<N>& hits, Rfit::Matrix6xNf<N>& hits_ge);
 * @brief Performs fast fit on the given hits
 *
 * @param[in] hits matrix of hits
 * @param[out] fast_fit_results vector of fitted parameters
 ...
 *
 * @param[in] hits matrix of hits
 * @param[in] fast_fit_results vector of initial fitted parameters
 ...
 * @brief Performs line fit on the given hits and prepared data
 *
 * @param[in] hits_ge matrix of hits covariance
 * @param[in] fast_fit_results vector of initial fitted parameters
 * @param[in] B curvature parameter
 * @param[in] data prepared broken line data
 * @param[out] line_fit_results structure containing fitted parameters and chi2
 ...
 * @brief Performs circle fit on the given hits and prepared data
 *
 * @param[in] hits matrix of hits
 * @param[in] hits_ge matrix of hits covariance
 * @param[in] fast_fit_results vector of initial fitted parameters
 * @param[in] B curvature parameter
 * @param[in] data prepared broken line data
 * @param[out] circle_fit_results structure containing fitted parameters and chi2
 ...
 * @brief Loads 2D covariance matrix from the given 6xN matrix
 *
 * @param[in] hits_ge matrix of hits covariance
 * @param[out] hits_cov 2D covariance matrix
 
void loadCovariance2D(const Rfit::Matrix6xNf<N>& hits_ge, Rfit::Matrix2Nd<N>& hits_cov);
 * @brief Computes radius from the given 2D points
 *
 * @param[in] points 2D points
 * @return vector of radii
 
Rfit::VectorNd<N> rad(const Rfit::Matrix2Nd<N>& points);
 * @brief Performs circle fit on the given points and covariance matrix
 *
 * @param[in] points 2D points
 * @param[in] cov covariance matrix
 * @param[in] init_guess initial guess for the fit
 * @param[in] rad vector of radii
 * @param[in] B curvature parameter
 * @param[in] use_uncertainties flag indicating whether to use uncertainties in the fit
 * @return structure containing fitted parameters and chi2
 
Rfit::circle_fit Circle_fit(const Rfit::Matrix2Nd<N>& points, const Rfit::Matrix2Nd<N>& cov, const Vector4d& init_guess, const Rfit::VectorNd<N>& rad, double B, bool use_uncertainties);
 * @brief Performs line fit on the given hits and circle fit results
 *
 * @param[in] hits matrix of hits
 * @param[in] hits_ge matrix of hits covariance
 * @param[in] circle_fit_results structure containing circle fit parameters and chi2
 * @param[in] fast_fit_results vector of initial fitted parameters
 * @param[in] B curvature parameter
 * @param[in] use_uncertainties flag indicating whether to use uncertainties in the fit
 * @return structure containing fitted parameters and chi2
 
Rfit::line_fit Line_fit(const Rfit::Matrix3xNd<N>& hits, const Rfit::Matrix6xNf<N>& hits_ge, const Rfit::circle_fit& circle_fit_results, const Vector4d& fast_fit_results, double B, bool use_uncertainties);
 * @brief Converts circle fit parameters to uvrt format
 *
 * @param[inout] circle_fit_results structure containing circle fit parameters and chi2
 * @param[in] B curvature parameter
 * @param[in] use_uncertainties flag indicating whether to use uncertainties in the conversion
 
void par_uvrtopak(Rfit::circle_fit& circle_fit_results, double B, bool use_uncertainties);*/void testFit() {
