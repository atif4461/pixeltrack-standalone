 ...
 * It uses random number generation to populate histograms and verifies their correctness.
...
assert(h.size() == N)
 * @brief Verify that the offset of the first bin in the histogram is zero
 *
 * This assertion checks that the offset of the first bin is correctly set to zero.
 
assert(h.off[0] == 0)
 * @brief Lambda function to verify the ordering of elements within bins
 *
 * This lambda function takes four parameters: indices i, j, k, and values t1 and t2.
 * It asserts that if i is not equal to j and the difference between v[t1] and v[t2] is less than or equal to zero,
 * then an error message is printed indicating a failure.
 *
 * @param i Index i
 * @param j Index j
 * @param k Index k
 ...
 * @brief Function to iterate over elements in bins
 *
 * This function iterates over elements in bins and applies a given function to each element.
 *
 * @param h Histogram object
 * @param val Value to search for
 * @param width Width of the bin range
