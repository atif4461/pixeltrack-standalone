 ...
 * It allows tasks to be added while another task is holding the resource and 
 * signals when the resource is available for the waiting tasks to proceed.
  
 * Class representing a node in the waiting task list.
 * Each node holds a task and a reference to the next node in the list.
  
 * Function announcing the availability of the resource and spawning the tasks.
 * This function is thread-safe and can be called from multiple threads.
  
 * Constructor initializing the WaitingTaskList with an optional initial size.
 * The initial size is used for optimization purposes and the list can resize itself.
 
 * Destructor releasing resources allocated by the WaitingTaskList.
  
 * Function adding a task to the waiting list.
 * Tasks can be added while another task is holding the resource and will be spawned 
 * when the resource becomes available.
 
 * Function signaling that the resource is now available and tasks should be spawned.
 * If a task fails, a non-null exception pointer should be passed to handle the error.
 
 * Function resetting access to the resource so that added tasks will wait.
 * This function is not thread-safe and should only be called when no tasks are using the resource.
  
 * Function creating a new node in the waiting task list.
 * Each node represents a task waiting for the resource to become available.
  
 * Data structure representing the head of the waiting task list.
 * This atomic pointer ensures thread safety when accessing the list.
  
 * Cache of nodes to improve performance by reducing dynamic memory allocation.
 * The cache size can be adjusted for optimization purposes.
  
 * Pointer to the last assigned node in the cache.
 * This atomic variable ensures thread safety when assigning nodes from the cache.
  
 * Flag indicating whether the list is currently waiting for the resource.
 * This atomic boolean ensures thread safety when checking the waiting state.
  
 * Exception pointer storing information about the last exception occurred.
 * This pointer is used to handle errors when a task fails while holding the resource.
