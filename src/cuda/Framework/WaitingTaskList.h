 ...
 * It allows tasks to be added while the resource is still busy and then spawn them once the resource is free.
 * The class also handles exceptions that occur during task execution.
  
 * Constructor initializing the WaitingTaskList object with an optional initial size parameter.
 * The initial size determines the number of tasks that can be cached before dynamic resizing occurs.
 
 * Adds a task to the waiting list without blocking, allowing concurrent adds and doneWaiting calls.
 * If doneWaiting has already been called, the task will be spawned immediately.
 
 * Alternative method to add a task to the waiting list, taking a WaitingTaskHolder instead of individual parameters.
 * Allows concurrent adds and doneWaiting calls.
 
 * Signals that the resource is now available, triggering the spawning of waiting tasks.
 * Accepts an optional exception pointer to handle failures, requiring a subsequent doneWaiting call with the same exception.
 * Enables concurrent adds and doneWaiting calls.
 
 * Resets access to the resource, causing newly added tasks to wait again.
 * Must be called when no tasks are using the resource and without concurrent add or doneWaiting calls.
 
 * Private method handling task spawning, designed to be thread-safe.
 
 * Nested struct representing a node in the waiting task list, containing task and group pointers, and atomic next node reference.
 * Includes methods for setting and retrieving the next node in the sequence.
 
 * Creates a new WaitNode instance with the specified task group and task, returning a pointer to the created node.
 
 * Data members of the WaitingTaskList class, including atomic head pointer, node cache, exception pointer, cache size, last assigned cache index, and waiting flag.
