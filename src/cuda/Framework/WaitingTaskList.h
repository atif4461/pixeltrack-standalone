 ...
 * and signals when the resource is available for the waiting tasks to start.
 ...
 * Adds a task to the waiting list, either immediately spawning it if the resource is available,
 * or holding it until the resource becomes available through a call to doneWaiting.
 ...
 * Signals that the resource is now available, allowing waiting tasks to start accessing it.
 * If an exception occurred, it can be passed to this function to notify waiting tasks.
 * Tasks can be made to wait again by calling reset.
 * Concurrent calls to add and doneWaiting are allowed.
 
 * Resets access to the resource, making newly added tasks wait until doneWaiting is called again.
 * This function is not thread-safe and should only be called when no tasks are using the resource and no concurrent calls to add or doneWaiting are being made.
 
 * Private helper function to spawn waiting tasks, safe to call from multiple threads.
 
 * Struct representing a node in the linked list of waiting tasks.
 * Each node contains a task, its associated task group, and a pointer to the next node in the list.
 
 * Creates a new node in the linked list for a given task and task group.
 
 * Member variable pointing to the head of the linked list of waiting tasks.
 
 * Cache of nodes to improve performance by reusing previously allocated memory.
 
 * Pointer to an exception that occurred while a task was holding the resource.
 
 * Size of the node cache.
 
 * Index of the last assigned node in the cache.
 
 * Flag indicating whether tasks are currently waiting for the resource to become available.
