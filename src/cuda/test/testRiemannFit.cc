 ...
 * input matrix. The values are assigned according to different conditions
 * based on the compile-time column count of the input matrix.
 *
 ...
 *
 * This function generates hits and their corresponding covariance matrices,
 * performs fast fit, circle fit, and line fit, and prints out the fitted parameters
 * and chi-squared values.
 
void testFit()
 * @brief Fills hit and hit covariance matrices with generated data.
 *
 * @param[out] hits Hit matrix filled with generated data.
 * @param[out] hits_ge Hit covariance matrix filled with generated data.
 ...
 *
 * @param[in] hits Hit matrix used to prepare broken line data.
 * @param[in] fast_fit_results Initial fit results used to prepare broken line data.
 * @param[in] B Magnetic field strength used in preparation of broken line data.
 ...
 * @brief Performs fast fit on the given hits.
 *
 * @param[in] hits Hits used for fast fit.
 * @param[out] fast_fit_results Fast fit results.
 
void Fast_fit(const Rfit::Matrix3xNd<N>& hits, Vector4d& fast_fit_results)
 * @brief Performs BL fast fit on the given hits.
 *
 * @param[in] hits Hits used for BL fast fit.
 * @param[out] fast_fit_results BL fast fit results.
 
void BL_Fast_fit(const Rfit::Matrix3xNd<N>& hits, Vector4d& fast_fit_results)
 * @brief Performs line fit on the given hits and covariance matrix.
 *
 * @param[in] hits_ge Covariance matrix used for line fit.
 * @param[in] fast_fit_results Initial fit results used for line fit.
 * @param[in] B Magnetic field strength used in line fit.
 * @param[in] data Prepared broken line data used for line fit.
 * @param[out] line_fit_results Line fit results.
 
void BL_Line_fit(const Rfit::Matrix6xNf<N>& hits_ge, const Vector4d& fast_fit_results, double B, const BrokenLine::PreparedBrokenLineData<N>& data, Rfit::line_fit& line_fit_results)
 * @brief Performs circle fit on the given hits and covariance matrix.
 *
 * @param[in] hits Hits used for circle fit.
 * @param[in] hits_ge Covariance matrix used for circle fit.
 * @param[in] fast_fit_results Initial fit results used for circle fit.
 * @param[in] B Magnetic field strength used in circle fit.
 * @param[in] data Prepared broken line data used for circle fit.
 * @param[out] circle_fit_results Circle fit results.
 
void BL_Circle_fit(const Rfit::Matrix3xNd<N>& hits, const Rfit::Matrix6xNf<N>& hits_ge, const Vector4d& fast_fit_results, double B, const BrokenLine::PreparedBrokenLineData<N>& data, BrokenLine::karimaki_circle_fit& circle_fit_results)
 * @brief Loads 2D covariance matrix from the given 6xN covariance matrix.
 *
 * @param[in] hits_ge 6xN covariance matrix used to load 2D covariance.
 ...
 *
 * @param[in] hits Hits used for circle fit.
 * @param[in] hits_cov Covariance matrix used for circle fit.
 * @param[in] fast_fit_results Initial fit results used for circle fit.
 * @param[in] rad Radii used for circle fit.
 * @param[in] B Magnetic field strength used in circle fit.
 * @param[in] isHelix Whether helix track model is used.
 * @param[out] circle_fit_results Circle fit results.
 
Rfit::circle_fit Circle_fit(const Rfit::Matrix2Nd<N>& hits, const Rfit::Matrix2Nd<N>& hits_cov, const Vector4d& fast_fit_results, const Rfit::VectorNd<N>& rad, double B, bool isHelix)
 * @brief Performs line fit on the given hits and covariance matrix.
 *
 * @param[in] hits Hits used for line fit.
 * @param[in] hits_ge Covariance matrix used for line fit.
 * @param[in] circle_fit_results Circle fit results used for line fit.
 * @param[in] fast_fit_results Initial fit results used for line fit.
 * @param[in] B Magnetic field strength used in line fit.
 * @param[in] isHelix Whether helix track model is used.
 * @param[out] line_fit_results Line fit results.
 
Rfit::line_fit Line_fit(const Rfit::Matrix3xNd<N>& hits, const Rfit::Matrix6xNf<N>& hits_ge, const Rfit::circle_fit& circle_fit_results, const Vector4d& fast_fit_results, double B, bool isHelix)
 * @brief Converts circle fit parameters to uvrtopa coordinates.
 *
 * @param[inout] circle_fit_results Circle fit results converted to uvrtopa coordinates.
 * @param[in] B Magnetic field strength used in conversion.
 * @param[in] isHelix Whether helix track model is used.
 
void par_uvrtopak(Rfit::circle_fit& circle_fit_results, double B, bool isHelix)*/void testFit() {
