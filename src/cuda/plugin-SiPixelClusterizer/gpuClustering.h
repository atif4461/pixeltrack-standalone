...
/**
 * Counts modules in a given array of identifiers.
 *
 * @param id          Array of identifiers to process.
 * @param moduleStart Array to store start indices of modules.
 * @param clusterId   Array to store cluster IDs.
...
/**
 * @brief Kernel function to find clusters in a module.
 *
 * This kernel function takes in various inputs such as module IDs, local coordinates,
 * and indices of the first pixel of each module. It outputs the number of clusters
 * found in each module, the module ID of each module, and the cluster ID of each pixel.
 *
 * The function iterates over the pixels in a module, skipping invalid pixels, and
 * finds the index of the first pixel not belonging to the current module. It then
 * initializes a histogram and fills it with the Y-coordinates of the pixels in the
 * module. The function also allocates space for duplicate pixels and finds the
 * nearest neighbors for each pixel.
 *
 * The function then enters a loop where it updates the cluster IDs of the pixels
 * based on their nearest neighbors. This process is repeated until no more changes
 * are made to the cluster IDs.
 *
 * Finally, the function finds the number of different clusters in the module,
 * marks the pixels in each cluster with a unique ID, and propagates this ID to all
