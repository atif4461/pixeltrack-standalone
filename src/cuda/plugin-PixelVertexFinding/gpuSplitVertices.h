 ...
 * if the chi-squared value is less than a certain threshold. If either condition is met, it skips the vertex.
 * For other vertices, it performs an iterative process to determine if the vertex should be split into two.
 * The decision to split is based on the distance between the tracks and the weights assigned to them.
 * If a split is decided, it updates the track indices and assigns them to the new intermediate vertex.
 *
 * Parameters:
 * pdata - Pointer to ZVertices structure containing vertex data
 * pws - Pointer to WorkSpace structure containing workspace data
 * maxChi2 - Maximum allowed chi-squared value
