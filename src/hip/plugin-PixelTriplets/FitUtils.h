 ...
 * @param prefix    An optional string to prepend to each line of output.
 ...
 *
 * @param ge Input matrix containing covariance elements.
...
/**
 * Brief description: Loads covariance from input ge into output hits_cov.
 *
 * Detailed description: This function takes an input matrix ge and loads its
 * elements into the output matrix hits_cov according to specific indexing rules.
 * The indexing rules are defined such that the input matrix ge is formatted as
 * [xx, xy, yy, xz, yz, zz] which corresponds to matrix elements
 * [(0,0), (0,1), (1,1), (0,2), (1,2), (2,2)]. The function iterates over each column
 * of the input matrix and assigns the corresponding values to the output matrix.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
 ...
 *
 * Detailed description: This function transforms the input parameters ip and icov
 * to the perigee plane and stores the results in op and ocov. It calculates the
 * sine and cosine of theta from the input parameter ip(3), then uses these values
 * to compute the output parameters op and the Jacobian matrix J. Finally, it
 * computes the covariance matrix ocov by multiplying the Jacobian matrix J with
 * the input covariance matrix icov and its transpose.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.5.
*/ 
