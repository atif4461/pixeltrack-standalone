...
/**
 * @brief Split vertices into two intermediate vertices based on track association.
 *
 * This function takes in a set of vertices and their associated tracks,
 * and splits them into two intermediate vertices if certain conditions are met.
 *
 * The splitting process involves an iterative method to determine the optimal
 * separation point between the two new vertices.
 *
 * @param pdata Pointer to the input vertex data structure
 * @param pws Pointer to the workspace data structure
 * @param maxChi2 Maximum allowed Chi2 value for vertex splitting
...
*/ 
  __device__ __forceinline__ void splitVertices(ZVertices* pdata, WorkSpace* pws, float maxChi2) {
    ...
    for (uint32_t kv = blockIdx.x; kv < nvFinal; kv += static_cast<uint32_t>(gridDim.x)) {
      if (nn[kv] < 4)
        continue;
      ...
      // copy to local
      for (uint32_t k = threadIdx.x; k < nt; k += static_cast<uint32_t>(blockDim.x)) {
        if (iv[k] == int(kv)) {
      ...
      while (__syncthreads_or(more)) {
        ...
        __syncthreads();
        for (uint32_t k = threadIdx.x; k < nq; k += static_cast<uint32_t>(blockDim.x)) {
          auto i = newV[k];
